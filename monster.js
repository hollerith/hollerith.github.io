...
(function ($root) {
    ...
    const Template = {
        ...
        render_pragmas: function (template) {
            ...
            return template.replace(regex, (match, pragma, options) => {
                if (!this.pragmas_implemented[pragma]) {
                    throw new Error(`This monster doesn't understand the '${pragma}' pragma`);
                }
                this.pragmas[pragma] = {};
                if (options) {
                ...
                if (type === "^") {
                    ...
                    if (Array.isArray(value)) {
                        renderedContent = value.map((row) => {
                            let currentContext = this.create_context(row);
                            if (content.includes("{{.}}")) {
                                currentContext = { ".": row };
                            }
                            return this.render(content, currentContext, partials, true);
                        }).join("");
        ...
        render_tags: function (template, context, partials, in_recursion) {
            const tag_regex = new RegExp(`${this.otag}([>#\/^!])?([^\/#^!]+?)\\1?${this.ctag}+`, "g");

            const lines = template.split("\n");
            for (let i = 0; i < lines.length; i++) {
                lines[i] = lines[i].replace(tag_regex, (match, operator, name) => {
                    name = name.trim();
                    switch (operator) {
                        case "!":
                            return "";
                        ...
                        default:
                            const value = this.find(name, context);
                            if (name === ".") {
                                return context.hasOwnProperty(".") ? context["."] : context;
                            }
    ...
    const Factory = {
        ...
        create: function ($context) {
            const detectedComponents = Factory.detect($context);
            detectedComponents.forEach(function (component) {
                const varName = Object.keys($context).find(key => $context[key] === component);
                if (!varName) return;

                const customTagElements = $context.document.getElementsByTagName(varName);
                let builtComponent;

                if (customTagElements.length > 0) {
                    Array.from(customTagElements).forEach(function (element) {
                        builtComponent = $context.document.body.$build(component, []);
                        element.parentNode.replaceChild(builtComponent, element);
                    });
                } else {
                    const elementById = $context.document.getElementById(component.id);
                    builtComponent = $context.document.body.$build(component, []);

                    if (elementById) {
                        elementById.parentNode.replaceChild(builtComponent, elementById);
                    } else {
                        $context.document.body.appendChild(builtComponent);
                    }
                }

                if (varName && builtComponent) {
                    $context[varName] = builtComponent;
                }
            });
        },
    };
